...
# This is our training loop...
    def update(self, dt):
        ...
        if done:
            ...
            # reset the env and get a fresh observation
            # only if env is reset by us
            if episode_timesteps == 10001:
                self.serve_car()
                ...
                patch = np.ones((40, 40))
                refX = int(self.ball1.x)
                refY = int(self.ball1.y)
                if refX > longueur:
                    refX = longueur
                ...
                if (refX + 20) >= (longueur):
                    src_toX = (longueur)
                    tgt_toX = 20 + (longueur - refX)
                else:
                    src_toX = refX + 20
                    tgt_toX = 40

                # take out a patch from self.ball1.y - 40 or 0 to self.ball1.y + 40 or largeur in y axis
                if refY < 20:
                    src_fromY = 0
                    tgt_fromY = 20 - refY
                else:
        ...
        # take out a patch from self.ball1.y - 20 or 0 to self.ball1.y + 40 or largeur in y axis
        if refY < 20:
            src_fromY = 0
            tgt_fromY = 20 - refY
        else:
...
#                            T.Resize(40, interpolation=PILImage.CUBIC),
#                            T.ToTensor()])
#        patchTensor = torch.Tensor(patch)
#        patchTensor = resize(patchTensor)
#        patch = patchTensor
        ...
        new_obs = patch.reshape(1, 40, 40)
        #new_obs = patch
        reward = last_reward
        # We check if the episode is done
        #done_bool = 0 if episode_timesteps + 1 == 1000 else float(done)
        if episode_timesteps == 10000:
            done = True

        # We check if the episode is done
        done_bool = 0 if episode_timesteps == 10000 else float(done)
